package com.wolf.android.http.cookie.store;
...
public class PersistentCookieStore implements CookieStore {
	...
	protected void add(HttpUrl uri, Cookie cookie) {
		...
		prefsWriter.putString(uri.host(),
				TextUtils.join(",", cookies.get(uri.host()).keySet()));
	...
	public boolean remove(HttpUrl uri, Cookie cookie) {
		...
		if (cookies.containsKey(uri.host())
				&& cookies.get(uri.host()).containsKey(name)) {
			cookies.get(uri.host()).remove(name);
