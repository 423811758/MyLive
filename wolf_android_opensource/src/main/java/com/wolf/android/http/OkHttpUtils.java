...
import com.wolf.android.http.builder.PostFileBuilder;
import com.wolf.android.http.builder.PostFormBuilder;
import com.wolf.android.http.builder.PostStringBuilder;
...
public class OkHttpUtils {
					...
					if (!finalCallback.validateReponse(response, id)) {
						sendFailResultCallback(call, new IOException(
								"request failed , reponse's code is : "
										+ response.code()), finalCallback, id);
						return;
					}
	...
	public void sendFailResultCallback(final Call call, final Exception e,
			final Callback callback, final int id) {
		if (callback == null)
			return;
